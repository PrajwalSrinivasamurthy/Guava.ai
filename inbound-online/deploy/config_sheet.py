"""
Loads Texas Tech Online FAQ content from a Google Sheet.

Expected sheet structure — this program's tab (settings.FAQ_SHEET_TAB, default "Online
...
(e.g. "K12 FAQs", "10K FAQs") rather than mixed into one tab with a program column —
each program's own project reads only its own tab.

On every successful load, a local cache is written to /tmp so the deployed project
directory can stay read-only. A committed seed copy of config_cache.json lives next to
this file and ships with the deploy bundle; it's used as a read fallback only when /tmp
is empty (e.g. the first poll cycle after a cold start, before any successful sheet
load has happened). The current seed was generated once from texas_tech_online_kb.md's
initial content, which started as a placeholder.
...
_RUNTIME_CACHE_PATH = "/tmp/inbound_online_faq_cache.json"
...
def _load_faqs(spreadsheet_id: str) -> list[dict]:
    ...
    for row in rows[1:]:  # skip header
        ...
        if not question or not answer:
            continue
        faqs.append({"category": category or "Texas Tech Online", "question": question, "answer": answer})
    return faqs
...
def apply_numbers(target, mapping: dict[str, str]) -> None:
    ...
    {"GRAD_NUMBER": "grad_live"}. An explicit env var for that attr always wins.
