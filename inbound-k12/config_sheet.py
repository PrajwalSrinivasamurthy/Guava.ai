"""
Loads Texas Tech K-12 FAQ content from a Google Sheet.

Expected sheet structure — this program's tab (settings.FAQ_SHEET_TAB, default "K12
...
(e.g. "Online FAQs", "Grad FAQs", "10K FAQs") rather than mixed into one tab with a
program column — each program's own project reads only its own tab. Note: legacy
Grace's K12 bot has its own attached knowledge base ("K12 (PRD)" / "K12 AH (PRD)"),
but we don't have that KB's actual content, so this is freshly authored content,
...
_RUNTIME_CACHE_PATH = "/tmp/inbound_k12_faq_cache.json"
...
def apply_numbers(target, mapping: dict[str, str]) -> None:
    ...
    {"LIVE_NUMBER": "k12_live"}. An explicit env var for that attr always wins.
...
def refresh_numbers(spreadsheet_id: str) -> dict[str, str]:
    ...
    for row in rows[1:]:  # skip header
        ...
        if not key or not number:
            continue
        # Sheets parses a leading "+" as a formula and drops it on write — defend on
        # read too.
