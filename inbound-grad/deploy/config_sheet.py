"""
Loads Texas Tech Graduate School FAQ content from a Google Sheet.

Expected sheet structure — this program's tab (settings.FAQ_SHEET_TAB, default "Grad
FAQs"). Columns are resolved by header name, not position, so "category" is optional:
      question | answer
      category | question | answer   (category, in any position, groups the rendered
                                       corpus; rows with it blank/absent fall back to
                                       settings.ORGANIZATION_NAME)

Other Texas Tech programs' FAQ content lives in separate tabs of the same spreadsheet
(e.g. "Online FAQs", "K12 FAQs", "10K FAQs") rather than mixed into one tab with a
program column — each program's own project reads only its own tab. Note: legacy
Grace's Grad bot has no knowledge base attached at all (unlike Online/K12/10K), so this
is net-new content, not a replication of anything legacy Grace answers.
...
_RUNTIME_CACHE_PATH = "/tmp/inbound_grad_faq_cache.json"
...
def _load_faqs(spreadsheet_id: str) -> list[dict]:
    # A:C covers "category | question | answer"; also works if the tab only has two
    # columns ("question | answer", no category) — column position is resolved from
    # the header row below, not assumed, so either shape reads correctly.
    ...
    try:
        ...
        raise RuntimeError(
            f"{settings.FAQ_SHEET_TAB!r} tab header must include 'question' and "
            f"'answer' columns (category is optional); got {rows[0]!r}"
        )
    category_idx = header.index("category") if "category" in header else None

    faqs = []
    for row in rows[1:]:  # skip header
        while len(row) <= max(question_idx, answer_idx, category_idx or 0):
            row.append("")
        question, answer = row[question_idx].strip(), row[answer_idx].strip()
...
def apply_numbers(target, mapping: dict[str, str]) -> None:
    ...
    {"LIVE_NUMBER": "grad_live"}. An explicit env var for that attr always wins.
