...
Texas Tech Graduate School's dedicated inbound line. Greets the caller and offers a
choice between continuing with the virtual assistant or being transferred to a live
team member (during business hours) — or continuing with the assistant or leaving a
voicemail (after hours) — then hands off accordingly. Matches legacy Grace's "Grad +
EL (Production)" / "Grad + EL AH (Production)" playbooks.
...
`.env.example` for `FAQ_SPREADSHEET_ID`/`FAQ_SHEET_TAB` (see `settings.py` for the default tab name).
No committed seed exists yet — until
...
```bash
./scripts/prepare-deploy.sh customers/texas_tech/implementations/inbound-grad
```
...
```bash
UV_CACHE_DIR="$TMPDIR/uv-cache" ./scripts/prepare-deploy.sh customers/texas_tech/implementations/inbound-grad
```
