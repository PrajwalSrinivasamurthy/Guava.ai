...
@pytest.fixture(autouse=True)
def _no_real_sms(monkeypatch, app):
    """Belt and braces: no test path should ever send a real text message."""
    import guava
    monkeypatch.setattr(guava, "Client", lambda *a, **k: pytest.fail("a test tried to send a real SMS"))
